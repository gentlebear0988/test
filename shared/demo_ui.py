...
Scaffold / product repos should keep a copy next to demo.py (same as license_ux.py).
...
def result_dataframe(*, headers: list[str] | None = None, label: str = "Extracted fields"):
    ...
    kw = dict(
        headers=headers or ["Field", "Value", "Source"],
        datatype=["str"] * len(headers or ["Field", "Value", "Source"]),
        interactive=False,
        wrap=True,
        label=label,
